...
export const schemaRegister =  Yup.object().shape({
    ...
    email: Yup.string().when([], (isRequired, schema) => {
        let baseSchema = schema
            .email(ErrorMessage.EMAIL.WRONG_EMAIL_FORMAT)
    ...
    rePassword: Yup.string().when([], (isRequired, schema) => {
        ...
        if(Condition.RE_PASSWORD.REQUIRED){
            baseSchema = baseSchema
                            .oneOf([Yup.ref('password'), undefined], ErrorMessage.RE_PASSWORD.NO_MATCH)
                            .nullable().required(ErrorMessage.RE_PASSWORD.REQUIRED)
        }
        return baseSchema;
    })
  });